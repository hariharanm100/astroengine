...
def recreate_interaccion(interaccion_df):
    ...
    while count <= 10:
        ...
        range_count += 1
    df = pd.DataFrame()
    df['INTERACCIÓN'] = interaccion_temp_lis
    df['puntaje'] = puntaje_temp_lis
    return df


def breakdown_interaccion(recreate_interaccion_df, request):
    df1 = recreate_interaccion_df.copy()
    if request.method == "POST":
        exdegree = request.POST["ex_degree"]
        if exdegree == "SU":
            temp_main_df = df1[0:10]
            sum_puntaje = sum(df1[0:10]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[0:10]['INTERACCIÓN'].to_list(), df1[0:10]['puntaje'].to_list()
        if exdegree == "MO":
            temp_main_df = df1[10:20]
            sum_puntaje = sum(df1[10:20]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[10:20]['INTERACCIÓN'].to_list(), df1[10:20]['puntaje'].to_list()
        if exdegree == "ME":
            temp_main_df = df1[20:30]
            sum_puntaje = sum(df1[20:30]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[20:30]['INTERACCIÓN'].to_list(), df1[20:30]['puntaje'].to_list()
        if exdegree == "MA":
            temp_main_df = df1[30:40]
            sum_puntaje = sum(df1[30:40]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[30:40]['INTERACCIÓN'].to_list(), df1[30:40]['puntaje'].to_list()
        if exdegree == "JU":
            temp_main_df = df1[40:50]
            sum_puntaje = sum(df1[40:50]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[40:50]['INTERACCIÓN'].to_list(), df1[40:50]['puntaje'].to_list()
        if exdegree == "VE":
            temp_main_df = df1[50:60]
            sum_puntaje = sum(df1[50:60]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[50:60]['INTERACCIÓN'].to_list(), df1[50:60]['puntaje'].to_list()
        if exdegree == "SA":
            temp_main_df = df1[60:70]
            sum_puntaje = sum(df1[60:70]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[60:70]['INTERACCIÓN'].to_list(), df1[60:70]['puntaje'].to_list()
        if exdegree == "RA":
            temp_main_df = df1[70:80]
            sum_puntaje = sum(df1[70:80]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[70:80]['INTERACCIÓN'].to_list(), df1[70:80]['puntaje'].to_list()
        if exdegree == "KE":
            temp_main_df = df1[80:90]
            sum_puntaje = sum(df1[80:90]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[80:90]['INTERACCIÓN'].to_list(), df1[80:90]['puntaje'].to_list()
        if exdegree == "AS":
            temp_main_df = df1[90:100]
            sum_puntaje = sum(df1[90:100]['puntaje'].to_list())
            temp_main_df = temp_main_df.append({'INTERACCIÓN': "TOTAL", 'puntaje': sum_puntaje}, ignore_index=True) 
            return temp_main_df, df1[90:100]['INTERACCIÓN'].to_list(), df1[90:100]['puntaje'].to_list()
        if exdegree == "ALL":
            return df1[0:100], df1[0:100]['INTERACCIÓN'].to_list(), df1[0:100]['puntaje'].to_list()
            



def exaspect_main(su_d, mo_d, me_d, ma_d, ju_d, ve_d, sa_d, ra_d, ke_d, as_d, su_d2, mo_d2, me_d2, ma_d2, ju_d2, ve_d2, sa_d2, ra_d2, ke_d2, as_d2,request):
    ...
    interaccion_result_df = breakdown_interaccion(recreate_interaccion_df, request) 
    ...
    breakdown_interaccion_df = dataframe_to_dict(interaccion_result_df[0])
    interaccion_lis = interaccion_result_df[1]
    puntaje_lis = interaccion_result_df[2]
    main_dic = {"eve_person_one": eve_person_one, "eve_person_two": eve_person_two, "house_df": house_df,
                ...
                "distance_planets_df": distance_planets_df, "interaccion_df": breakdown_interaccion_df, "interaccion_lis": interaccion_lis, "puntaje_lis":puntaje_lis}
    return main_dic
